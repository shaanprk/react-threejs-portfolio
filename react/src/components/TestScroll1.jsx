...
//     // console.log("[TestScroll1] pointer move", e);
//     updateRayCaster(e);
...
//       console.log("[TestScroll1] set cursor to pointer");
//     } else {
//       setHovered(false);
//       gl.domElement.style.cursor = "auto";
//       console.log("[TestScroll1] set cursor to auto");
//     }
//   };
...
//   // These functions remain for handling click and drag behavior
//   const updateRayCaster = (e) => {
//     mouse.x = (e.clientX / window.innerWidth) * 2 - 1;
//     mouse.y = -(e.clientY / window.innerHeight) * 2 + 1;
//     raycaster.setFromCamera(mouse, camera);
...
//   // Retain global listeners for click and mousedown (if needed)
//   useEffect(() => {
//     gl.domElement.addEventListener("click", handlePointerClickDefault);
//     gl.domElement.addEventListener("mousedown", handleMouseDown);
...
//         scale={1}
//         position={[0, 0, 0]}
//         // Use R3F pointer events to change the cursor when hovering over the scroll.
//         onPointerOver={(e) => {
//           e.stopPropagation();
...
const TestScroll1 = ({ page, isActive, onOpen }) => {
  ...
  // Remove global "click" listener in favor of the primitive's onClick.
  const handleMouseDown = (e) => {
    if (isOpenRef.current) {
      console.log("Scroll is open. Preventing dragging.");
      return;
  ...
  const handleMouseMove = (e) => {
    if (isDragging.current) {
      ...
      const deltaY = e.clientY - initialMouse.current.y;
      const [rx, ry, rz] = initialRotation.current;
      pivotRef.current.rotation.set(rx + deltaY * 0.01, ry + deltaX * 0.01, rz);
    }
  };
  ...
  // This useFrame block interpolates the scroll's pivot towards the target position and rotation.
  useFrame(() => {
    if (pivotRef.current && !isDragging.current) {
    ...
    mixer.update(0.016); // roughly 60 FPS
  });

  // Apply glow effect on string when centered
  useEffect(() => {
    if (texturedKnotRef.current) {
      if (isCentered) {
        texturedKnotRef.current.material.emissive = new THREE.Color(0xffffff);
  ...
  // Click handler on the scroll primitive.
