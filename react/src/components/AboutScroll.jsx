...
//         setIsCentered(true);
//         onCenter(page);
//         console.log(`Updating URL to: /?centered=${page}`);
//       } else {
//         // onCenter("");
//         onUncenter();
//         setIsCentered(false);
...
const AboutScroll = ({ page, isActive, onCenter, onUncenter, onKnotClick, resetTrigger }) => {
  // Load the scroll model
  const gltf = useLoader(GLTFLoader, scrollModel);
  const { scene, animations } = gltf;
  ...
  // Global pointer handlers (omitted for brevity)
  ...
  useEffect(() => {
    ...
    if (isActive) {
      if (isOpen && backgroundRef.current && scrollBox) {
        ...
        const frustumHeight = 2 * desiredDistance * Math.tan(verticalFOVRad / 2);
        const r = size.height / size.width;
        const fraction = r > 1 ? 0.61 - 0.1 * (r - 1.63) : 1.085 - (r - 0.5);
        targetY = fraction * frustumHeight;
      } else {
        targetZ = camera.position.z - computedD;
      }
  ...
  }, [isActive, isOpen, scrollSize, size, camera.fov, camera.aspect, camera.position.z, scrollBox]);
  ...
  return (
    <Fragment>
      ...
      <mesh ref={bgPlaneRef} onPointerDown={(e) => e.stopPropagation()}>
        <planeGeometry args={[100, 100]} />
        <meshBasicMaterial transparent opacity={0} />
      </mesh>
